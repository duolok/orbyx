...
install_package() {
  ...
  if [[ -d "$package_dir" && -f "$package_dir/setup.py" ]]; then
    ...
    cd "$package_dir" || exit
    python3 setup.py install || { echo "Installation failed for $package_dir"; read -p "Press any key to continue" x; exit 1; }
    cd - || exit
  else
...
start_server() {
  ...
  if [[ -d "$project_dir" ]]; then
    ...
    cd "$project_dir" || exit
    python manage.py runserver >> server.log 2>&1 || { echo "Failed to start Django server in $project_dir"; exit 1; }
    cd - || exit
  else
